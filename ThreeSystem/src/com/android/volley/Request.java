...
import com.android.volley.plus.CookieManager;
...
public abstract class Request<T> implements Comparable<Request<T>> {
	...
	/**
	 * 设置headers, 添加cookies Returns a list of extra HTTP headers to go along
	 * with this request. Can throw {@link AuthFailureError} as authentication
	...
	public Map<String, String> getHeaders() throws AuthFailureError {
		...
		map.put("Cookie", CookieManager.REQUEST_COOKIE);
		return map;
	}
	...
	protected Response<T> parseNetworkResponse(NetworkResponse response) {
		...
		for (String header : responseHeaders.keySet()) {
			if (header.contains("Set-Cookie")) {
				CookieManager.RESPONSE_COOKIE = responseHeaders.get(header);
				String[] cookies = responseHeaders.get(header).split(";");
				for (String cookie : cookies) {
					/*ASP.NET_SessionId PHPSESSID JSESSIONID*/
					if (cookie.contains(CookieManager.SESSION_NAME)) {
						if (!CookieManager.REQUEST_COOKIE.equals(cookie))
							CookieManager.REQUEST_COOKIE = cookie;
					}
					break;
				}
				break;
