...
public class CusImageView extends ImageView {
	...
	public CusImageView(Context context, int w, int h)// 这里传进来的w，h就是图片的移动范围
	{
		super(context);
	...
	public boolean onTouchEvent(MotionEvent event) {
		switch (event.getAction() & MotionEvent.ACTION_MASK) {// MotionEvent.ACTION_MASK
																// 表示多点触控事件
		...
		case MotionEvent.ACTION_UP:
			...
			if((float)getHeight()/getWidth()>(float)screenH/screenW){
				while (getHeight() < screenH) {
					setScale(scale, BIGGER);
				}
			}else{
				while (getWidth() < screenW) {
					setScale(scale, BIGGER);
				}
			}
			
			while (getHeight() > 2*screenH || getWidth() > 2*screenW) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mode == DRAG) {
				...
				if (spacing(event) > 10f) {
					...
					// 图片宽度（也就是自定义imageView）必须大于250才可以缩放
					else if (Math.abs(gapLenght) > 5f && getWidth() > 200) {
						if (gapLenght > 0) {
