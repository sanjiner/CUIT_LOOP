...
public class EditPage extends FakeActivity implements OnClickListener,
	...
	public void onCreate() {
		...
		new Thread() {
			...
			public void run() {
				...
				UIHandler
						.sendEmptyMessage(MSG_PLATFORM_LIST_GOT, EditPage.this);
			}
		}.start();
	}
	...
	private void initPageView() {
		...
		FrameLayout.LayoutParams lpPin = new FrameLayout.LayoutParams(dp_80,
				dp_36);
		lpPin.topMargin = cn.sharesdk.framework.utils.R
				.dipToPx(getContext(), 6);
		...
		llInput.setMinimumHeight(cn.sharesdk.framework.utils.R.dipToPx(
				getContext(), 150));
	...
	private void checkAtMth(LinearLayout llInput, String platform) {
		if ("SinaWeibo".equals(platform) || "TencentWeibo".equals(platform)
			...
			FrameLayout.LayoutParams lpAt = new FrameLayout.LayoutParams(
					android.view.ViewGroup.LayoutParams.WRAP_CONTENT,
					android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
			lpAt.leftMargin = cn.sharesdk.framework.utils.R.dipToPx(
					getContext(), 10);
			lpAt.bottomMargin = cn.sharesdk.framework.utils.R.dipToPx(
					getContext(), 10);
			...
			String text = getContext().getString(R.string.list_friends,
					getName(platform));
			tvName.setText(text);
	...
	public void afterPlatformListGot() {
		...
		final int dp_36 = cn.sharesdk.framework.utils.R.dipToPx(getContext(),
				36);
	...
	private Bitmap getPlatLogo(Platform plat) {
		...
		int resId = cn.sharesdk.framework.utils.R.getResId(R.drawable.class,
				resName);
