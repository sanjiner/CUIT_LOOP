...
public class LabelController extends BaseController{
	...
	private Page getLabelInfoList(String queryString, String applyBh, Integer pageIndex){
		...
		/*else if(queryString == null || queryString.equals("time")){
			sqlString = "from Labermanagement as lmodel where lmodel.applyOriginCode='"+applyBh+"'and lmodel.labelQueryId like '%"+queryString+"%'order by lmodel.makeTime desc";
			
		}
		else if(queryString == null || queryString.equals("label"))
			{
			sqlString = "from Labermanagement as lmodel where lmodel.applyOriginCode='"+applyBh+"'and lmodel.labelQueryId like '%"+queryString+"%'order by lmodel.labelStatus desc";
			}
		else if(queryString == null || queryString.equals("scan"))
		{
		...
		else if(queryString == null || queryString.equals("hasException"))
		{
			sqlString = "from Labermanagement as lmodel where lmodel.applyOriginCode='"+applyBh+"'and lmodel.labelQueryId like '%"+queryString+"%'order by lmodel.abnormal desc";
		}*/
	...
	//分页查询降序排序
	private Page getPageByOrder(String queryString,String applyBh,Integer pageIndex,String order)
	{
		Page page;
		String sqlString = "";
		if (pageIndex == null){
		...
		// 排序
			 ...
			 if(order.equals("time")){
				sqlString = "from Labermanagement as lmodel where lmodel.applyOriginCode='"+applyBh+"'order by lmodel.makeTime desc";
				
			}
			else if(order.equals("label"))
				{
				sqlString = "from Labermanagement as lmodel where lmodel.applyOriginCode='"+applyBh+"'order by lmodel.labelStatus desc";
				}
			else if( order.equals("scan"))
			{
				sqlString = "from Labermanagement as lmodel where lmodel.applyOriginCode='"+applyBh+"'order by lmodel.scanNum desc";
			}
			else if( order.equals("hasException"))
			{
				sqlString = "from Labermanagement as lmodel where lmodel.applyOriginCode='"+applyBh+"'order by lmodel.abnormal desc";
			}
			else {
				sqlString = "from Labermanagement as lmodel where lmodel.applyOriginCode='"+applyBh+"'order by lmodel.scanNum desc";
			}
	...
	public String orderBykey(@RequestParam(required = false) String order,@RequestParam("key") String pid,Model model
			,@RequestParam(required = false) Integer requestPageNo)
	{
		Page page = getPageByOrder( pid,order,requestPageNo);
	...
	public String applyLook(@RequestParam(required = false) Integer requestPageNo, 
		...
		Page page=getPageByOrder(queryString, pid, requestPageNo,order);
		boolean isPaging = page.getPageList().size() > 0 ? true : false; // 是否分页
		model.addAttribute("DataList", page);
		model.addAttribute("isPaging", isPaging);
