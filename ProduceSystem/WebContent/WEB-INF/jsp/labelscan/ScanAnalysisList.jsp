...
function goToPage() {
	...
	if(parseInt(totalPage) < parseFloat(document.getElementById("page").value) || parseFloat(document.getElementById("page").value) < 1){
		...
		var url = "<%=basePath%>ScanAnalysis/list?requestPageNo="
...
<div style="background: #4c9ee7;width: 100%;height: 30px;margin-bottom: 5px">
	<label style="width: auto;height: 30px;text-align: center;color: white;line-height: 30px;margin-left: 5px">认证标签管理>>二维码扫描分析>>认证产品列表</label>
</div>
<br/>
<div class="container">
		<div class="" style="margin: 0 auto;">
			<table>
				<tr>
					<td>
						<form name="form1" action="<%=basePath%>ScanAnalysis/list" method="post">
							&nbsp;&nbsp;&nbsp; 产品名称：
					...
					<tbody>
						<c:forEach var="data" items="${DataList.pageList}">
							<tr>
								...
								<td>气候品质认证</td>
								<td>
									<a href="<%=basePath%>ScanAnalysis/show?originID=${data.applyBh}&name=${data.productName}&time=${data.applyTime}">查看</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
					...
					<tbody>
						<c:forEach var="data" items="${DataList.pageList}">
							<tr>
								...
								<td>农网认证</td>
								<td>
									<a href="<%=basePath%>ScanAnalysis/show?originID=${data.originId}&name=${data.originName}&time=${data.applyTime}">查看</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
			...
			<span>第<font color="red">${DataList.currentPage}</font>页/共<font
				...
				color="red">${DataList.allSize}</font>条记录
					...
					color="red">${DataList.currentPageSize}</font>条数据]
						...
						<a
							href="<%=basePath%>ScanAnalysis/list?requestPageNo=${DataList.prePage}&queryString=${queryString}&applyType=${applyType}">上一页</a>
					</c:if>
			</span> <span> <c:if test="${DataList.nextPage != 0}">
						<a
							href="<%=basePath%>ScanAnalysis/list?requestPageNo=${DataList.nextPage}&queryString=${queryString}&applyType=${applyType}">下一页</a>
					</c:if>
