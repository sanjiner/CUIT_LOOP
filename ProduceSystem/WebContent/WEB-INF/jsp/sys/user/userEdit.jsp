...
<body>
	...
	<div class="container">
		<div style="margin-top:20px;">
			<form:form id="userForm" action="${ctx}sys/user/update" method="post"
				modelAttribute="user">
				<table class="tablecontainer">
					<tr>
						...
						<td>${user.cuitMoonUserName }<input type="hidden"
							name="userId" value="${user.cuitMoonUserId}">
						</td>
					</tr>
					<tr>
						<td>
					...
					<tr>
						...
						<td>
							<form:select id="cuitMoonAreaId" path="cuitMoonAreaId">
								<c:forEach items="${areaList}" var="areaPro">
									<option <c:if test="${user.cuitMoonAreaId == areaPro.cuitMoonAreaCode}">selected="selected"</c:if> value="${areaPro.cuitMoonAreaCode}">${areaPro.cuitMoonAreaName}</option>
									<c:forEach items="${areaPro.childAreas}" var="areaCity">
										<option <c:if test="${user.cuitMoonAreaId == areaCity.cuitMoonAreaCode}">selected="selected"</c:if> value="${areaCity.cuitMoonAreaCode}">&nbsp;&nbsp;${areaCity.cuitMoonAreaName}</option>
										<c:forEach items="${areaCity.childAreas}" var="areaCountry">
											<option <c:if test="${user.cuitMoonAreaId == areaCountry.cuitMoonAreaCode}">selected="selected"</c:if> value="${areaCountry.cuitMoonAreaCode}">&nbsp;&nbsp;&nbsp;&nbsp;${areaCountry.cuitMoonAreaName}</option>
										</c:forEach>
									</c:forEach>
								</c:forEach>
							</form:select>
					...
					<tr>
						...
						<td><form:select path="cuitMoonUserStatus"
								...
								<c:choose>
									<c:when test="${user.cuitMoonUserStatus eq 1}">
		 ...
		 function getCounty(code){
			 if(code == "0"){
				 ...
				 $.get("<%=basePath%>sys/area/getAreaJson?parentCode="+code,function(data){			
						$("#county").empty();
							for(var i= 0; i<data.length; i++){
								 $("#county").append("<option value=" + data[i].cuitMoonAreaCode + ">" + data[i].cuitMoonAreaName + "</option>");
							}
						}); 
			 }
