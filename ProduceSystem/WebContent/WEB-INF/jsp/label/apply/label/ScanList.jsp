...
function back(){
	var apId = document.getElementById('key').value;
...
function goToPage() {
	...
	if(parseInt(totalPage) < parseFloat(document.getElementById("page").value) || parseFloat(document.getElementById("page").value) < 1){
		...
		var url = "<%=basePath%>label/apply/label/scanlist?requestPageNo="
...
<div class="container">
		<div class="" style="margin: 0 auto;">
			<table>
				<tr>
					<td>
						<form name="form1" action="<%=basePath%>label/apply/label/scanlist" method="post">
							<input type="hidden" id="key" name="key" value="${applyId}">
			...
			<span>第<font color="red">${DataList.currentPage}</font>页/共<font
				...
				color="red">${DataList.allSize}</font>条记录
					...
					color="red">${DataList.currentPageSize}</font>条数据]
						...
						<a
							href="<%=basePath%>label/apply/label/scanlist?requestPageNo=${DataList.prePage}
							&queryString=${queryString}&key=${applyId}&Id=${labelId}&productId=${rpoductId}">上一页</a>
					</c:if>
			</span> <span> <c:if test="${DataList.nextPage != 0}">
						<a
							href="<%=basePath%>label/apply/label/scanlist?requestPageNo=${DataList.nextPage}
							&queryString=${queryString}&key=${productId}&Id=${labelId}&productId=${rpoductId}">下一页</a>
					</c:if>
