...
function goToPage() {
	...
	if(parseInt(totalPage) < parseFloat(document.getElementById("page").value) || parseFloat(document.getElementById("page").value) < 1){
		...
		var url = "<%=basePath%>label/apply/applylist?requestPageNo="
...
<div style="background: #4c9ee7;width: 100%;height: 30px;margin-bottom: 5px">
	<label style="width: auto;height: 30px;text-align: center;color: white;line-height: 30px;margin-left: 5px">认证标签管理>>标签申请>>认证产品列表</label>
</div>
<br/>
<div class="container">
		<div class="" style="margin: 0 auto;">
			<table>
				<tr>
					<td>
						<form name="form1" action="<%=basePath%>label/apply/applylist" method="post">
							&nbsp;&nbsp;&nbsp; 产品名称：
					...
					<tbody>
						<c:forEach var="data" items="${DataList.pageList}">
							<tr>
								...
								<td>气候品质认证</td>
								<td>
									<a href="<%=basePath%>label/apply/add?key=${data.applyBh}&type=${applyType}">申请</a> 					
								</td>
								<td>
									<a href="<%=basePath%>label/apply/look?key=${data.applyBh}&type=${applyType}">查看</a>
								</td>
								<td>
									<a href="<%=basePath%>label/apply/PreSee?key=${data.applyBh}&type=${applyType}">溯源预览</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
					...
					<tbody>
						<c:forEach var="data" items="${DataList.pageList}">
							<tr>
								...
								<td>农网认证</td>
								<td>
									<a href="<%=basePath%>label/apply/add?key=${data.originId}&type=${applyType}">申请</a> 					
								</td>
								<td>
									<a href="<%=basePath%>label/apply/look?key=${data.originId}&type=${applyType}">查看</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
			...
			<span>第<font color="red">${DataList.currentPage}</font>页/共<font
				...
				color="red">${DataList.allSize}</font>条记录
					...
					color="red">${DataList.currentPageSize}</font>条数据]
						...
						<a
							href="<%=basePath%>label/apply/applylist?requestPageNo=${DataList.prePage}&queryString=${queryString}&applyType=${applyType}">上一页</a>
					</c:if>
			</span> <span> <c:if test="${DataList.nextPage != 0}">
						<a
							href="<%=basePath%>label/apply/applylist?requestPageNo=${DataList.nextPage}&queryString=${queryString}&applyType=${applyType}">下一页</a>
					</c:if>
