...
function goToPage() {
	...
	if(parseInt(totalPage) < parseFloat(document.getElementById("page").value) || parseFloat(document.getElementById("page").value) < 1){
		...
		var url = "<%=basePath%>label/manage/application/list?requestPageNo="
...
<div style="background: #4c9ee7;width: 100%;height: 30px;margin-bottom: 5px">
	<label style="width: auto;height: 30px;text-align: center;color: white;line-height: 30px;margin-left: 5px">认证标签管理>>标签管理>>管理申请</label>
</div>
<br/>
<div class="container">
		<div class="" style="margin: 0 auto;">
			<table>
				<tr>
					<td>
						<form name="form1" action="<%=basePath%>label/manage/application/list" method="post">
					...
					<tbody>
						<c:forEach var="data" items="${DataList.pageList}">
							<tr>
								...
								<td>
									<c:if test="${data.labelMakeStatus == '未制作'}">
										<a href="<%=basePath%>label/manage/application/make?key=${data.labelApplicationId}">制作</a>&nbsp;&nbsp;
									</c:if>
									<c:if test="${data.labelMakeStatus == '制作完成'}">
										<a href="<%=basePath%>label/manage/application/down?key=${data.labelApplicationId}">下载</a>&nbsp;&nbsp;
									</c:if>									
									<a href="<%=basePath%>label/manage/application/edit?Id=${data.labelApplicationId}">编辑</a>&nbsp;&nbsp;
									<a href="<%=basePath%>label/manage/application/delete?key=${pid}&Id=${data.labelApplicationId}">删除</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
			...
			<span>第<font color="red">${DataList.currentPage}</font>页/共<font
				...
				color="red">${DataList.allSize}</font>条记录
					...
					color="red">${DataList.currentPageSize}</font>条数据]
						...
						<a
							href="<%=basePath%>label/manage/application/list?requestPageNo=${DataList.prePage}&queryString=${queryString}&key=${pid}">上一页</a>
					</c:if>
			</span> <span> <c:if test="${DataList.nextPage != 0}">
						<a
							href="<%=basePath%>label/manage/application/list?requestPageNo=${DataList.nextPage}&queryString=${queryString}&key=${pid}">下一页</a>
					</c:if>
