...
<br/>
<form id="form1" action="<%=basePath%>label/manage/label/edit" method="post">	
	<div>
		...
		<table>	
			<tr>
				...
				<td>
				<c:choose>
				<c:when test="${sessionScope.loginUser.getCuitMoonUserName() eq 'superadmin'}">
				<input type="text" id="scanNum" name="scanNum" value="${scanNum}"/>
				</c:when>
				<c:otherwise>
				<input type="text" id="scanNum" name="scanNum" value="${scanNum}" disabled="true"/>
				</c:otherwise>
				</c:choose>
				<%-- <input type="text" id="scanNum" name="scanNum" value="${scanNum}"/> --%><span style="color: #FF0000">*</span></td>
			</tr>
			<tr>
				<td>标签状态</td>
				<td>
