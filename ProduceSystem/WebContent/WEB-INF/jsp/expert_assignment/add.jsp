<%@page import="edu.cuit.module.auchc.entity.Expertsgrade"%>
...
<script type="text/javascript">
	...
	function addValidate() {
		...
		$(document).ready(function(){
			getMembers("${experts.get(0).expertsId}");
	});
	function getMembers(expertsId){
	$.get("<%=basePath%>expert_assign/getMembers?expertsId="
								+ expertsId,
	...
	function doSubmit() {
		...
		if(!contains(members,grouper,true)){
		 ...
		 $.ajax({
             type: "post",
             url: "<%=basePath%>expert_assign/add",
             dataType: "html",
             data: {"applyBh":applyBh,"members": members ,"groupName" :$("#textGroupName").text(),"groupRemarks":$("#textMemo").val(),
            	 "grouper":grouper},
             success:function(data,status){
            	 var obj = eval('(' + data + ')');
            	 if(obj.success == "true"){
            		 alert("添加成功");
            		 window.location.href = "<%=basePath%>expert_assign/list?applyBh="
									+ applyBh;
						} else {
							alert("添加失败");
						}
...
<body style="background: #edf1fc">
	...
	<label style="width: auto;height: 30px;text-align: center;color: white;line-height: 30px;margin-left: 5px">应用中心>>专家分配>>分配成员</label>
	</div>
	<input type="hidden" value="${experts.size()}" id="a"/>
	<table style="margin-top: 5px;border: 5px">
	<c:choose>
	<c:when test="${experts.size()==0}">
		<tbody>
				<tr>
					<td class="left">提示:</td>
					<td class="right" id="textGroupName">暂未建立专家组</td>
				</tr>
				<tr>
					<td class="right">
						<a type="button" class="btn_blue"
						href="<%=basePath %>expert_assign/list?applyBh=${applyBh}">取消</a>
					</td>
				</tr>
		</tbody>
		...
		<c:otherwise>
			...
			<tr>
				<td class="left">提示:</td>
				<td class="right" id="textGroupName">该项目已经通过省局审核，已在进行专家组分配 </td>
			</tr>
			<tr>
				<td class="left">专家组名称:</td>
				<td class="right" id="textGroupName">${experts.get(0).expertsName }</td>
			</tr>
			<tr>
				<td class="left">专家组层次:</td>
				<td class="right">${experts.get(0).expertsLevel }</td>
			</tr>
			<tr>
				...
				<td class="right"><select style="width: 100%"
					id="group_experts" onchange="group_change()">
						<c:forEach items="${experts}" var="expert">
							<option value="${expert.expertsId }">${expert.expertsName}</option>
						</c:forEach>
				</select></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="right"><a type="button" class="btn_blue"
					href="<%=basePath %>expert_assign/list?applyBh=${applyBh}">取消</a></td>
			</tr>
		</tbody>
		</c:otherwise>
		</c:choose>
