...
<script type="text/javascript" language="javascript">
		...
		//if(validate()){
       	...
       	if($("#mailBox").val().length > 0 && !( /^([a-zA-Z0-9_-]){1,}@([a-zA-Z0-9_-]){1,}((\.[a-zA-Z0-9_-]{2,3}){1,2})$/.test($("#mailBox").val()))){
			...
			if($("#provinceSelect").find("option:selected").val() != 0 && $("#citySelect").find("option:selected").val() != 0 && $("#countySelect").find("option:selected").val() != 0){
				...
				var url = "<%=basePath%>auth_center_info/business_info/update";
				$.ajax({
					type : "post",
					url : url,
					dataType : "html",
					data : { 'id': $("#primaryKey").val(),'userName': $("#userName").text(),'companyName': $("#campanyName").val(),'companyType': $("#companyType").find("option:selected").val(),'enterpriseType': $("#enterpriseType").find("option:selected").val(),'shopType': $("#shopType").find("option:selected").val(),'area': area,'legalPerson':$("#legalPerson").val(),'legalPresentCode':$("#legalPresentCode").val(),'cantactPerson':$("#cantactPerson").val(),'tel':$("#tel").val(),'mobilePhone':$("#mobilePhone").val(),'fax':$("#fax").val(),'mailBox':$("#mailBox").val(),'address':$("#address").val(),'logo':logo,'certificateName':$("#certificateName").val(),'awardDepart':$("#awardDepart").val(),'certifyImage': certifyImage,'getTime':$("#getTime").val(),'json':jsonStr,'remarks':$("#remarks").text()},
					success : function(data) {
						if(data == "T"){
	...
	function deleImg(btnDel){
		var url = "<%=basePath%>auth_center_info/business_info/deleteCertify";
		$.ajax({
			type : "get",
			url : url,
			dataType : "html",
			data : { 'id': $(btnDel).attr("id")},
			success : function(data) {
				if(data == "T"){
...
<body>
	<form name="form1" id="form1" method="post">
		...
		<table class="info" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td class="value"><span id="userName">${userName}</span></td>
			</tr>
			<tr>
				<td class="text"><span style="color: #FF0000">*</span>单位名称</td>
				<td class="value"><input type="text" id="campanyName" name="campanyName" value = "${companyName}"  class="width60per" ></td>
			</tr>
			<tr>
				<td class="text">单位性质</td>
				<td class="value"><select id="companyType">
						<c:forEach var="companyTypeVar" items="${companyTypeList}">
							<option value="${companyTypeVar.getCuitMoonDictionaryCode()}"  ${companyTypeVar.getCuitMoonDictionaryCode().toString()==companyType?'selected':''}>${companyTypeVar.getCuitMoonDictionaryName()}</option>
						</c:forEach>
				</select></td>
			</tr>
			<tr>
				...
				<td class="value">
				<select id="enterpriseType"  class="width60per">
						<c:forEach var="enterpriseTypeVar" items="${enterpriseTypeList}">
							<option value="${enterpriseTypeVar.getCuitMoonDictionaryCode()}" ${enterpriseTypeVar.getCuitMoonDictionaryCode().toString()==enterpriseType?'selected':''}>${enterpriseTypeVar.getCuitMoonDictionaryName()}</option>
						</c:forEach>
				</select>
				</td>
			</tr>
			<tr>
				...
				<td class="value"><select id="shopType"  class="width60per">
						<c:forEach var="shopTypeVar" items="${shopTypeList}">
							<option value="${shopTypeVar.getCuitMoonDictionaryCode()}" ${shopTypeVar.getCuitMoonDictionaryCode().toString()==shopType?'selected':''}>${shopTypeVar.getCuitMoonDictionaryName()}</option>
						</c:forEach>
				</select></td>
			</tr>
			<tr>
				...
				</select>  <select class="width25per" id = "citySelect" name="city" onchange="changeSelect(this.value,$('#countySelect'));">
				...
				</select>  <select class="width25per" id = "countySelect" name="county">
			...
			<tr>
				...
				<td class="value"><input type="text" id="legalPerson"  name = "legalPerson" value = "${legalPerson}"  class="width60per"></td>
			</tr>
			<tr>
				<td class="text"><span style="color: #FF0000">*</span>法人代表码</td>
				<td class="value"><input type="text" id="legalPresentCode" name="legalPresentCode" value = "${legalPresentCode}"  class="width60per"></td>
			</tr>
			<tr>
				<td class="text">通讯地址</td>
				<td class="value"><input type="text" id="address" name="address" value = "${address}"  class="width60per"></td>
			</tr>
			<tr>
				<td class="text">业务联系人</td>
				<td class="value"><input type="text" id="cantactPerson" name="cantactPerson" value = "${cantactPerson}"  class="width60per"></td>
			</tr>
			<tr>
				<td class="text">联系电话</td>
				<td class="value"><input type="text" id="tel" name="tel" value = "${tel}"></td>
			</tr>
			<tr>
				<td class="text">手机</td>
				<td class="value"><input type="text" id="mobilePhone" name="mobilePhone" value = "${mobilePhone}"  class="width60per"></td>
			</tr>
			<tr>
				<td class="text">传真</td>
				<td class="value"><input type="text" id="fax" name="fax" value = "${fax}"  class="width60per"></td>
			</tr>
			<tr>
				<td class="text">企业邮箱</td>
				<td class="value"><input type="text" id="mailBox" name="mailBox" value = "${mailBox}"  class="width60per"></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="value">
					<textarea id="remarks" name="remarks" rows="8" cols="25">${remarks}</textarea>
				</td>
			</tr>
			<tr>
				<td class="text"><hr /></td>
			...
			<tr>
				<td class="text">&nbsp;</td>
				<td class="value"><input type="button" value="提交" onclick="submitToServer()"  class="btn_blue" >&nbsp;&nbsp;&nbsp;
				<input type="button" value="返回"  class="btn_blue"  class="btn_blue" onclick="back()"></td>
			</tr>
		</table>
	...
	<script type="text/javascript">
        ...
        $("#uploadImage").uploadify({
            ...
            uploader: '<%=basePath%>auth_center_info/expert_info/file;jsessionid=<%=request.getSession().getId()%>',
			//文件选择后的容器ID  
			queueID : 'fileQueue',
			width : 120,
			auto : true,
