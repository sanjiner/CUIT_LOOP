...
<script type="text/javascript">
	...
	function submit(){
		...
		$("#imgCet").children().each(function(){
			imgCet += '/Upload/PicturePath/'+$(this).attr("id")+"|";
		});
		//拼接多语言数据
		...
		$("#uploadCetificate").uploadify({
			...
			onUploadSuccess : function(file, data, response) {
				...
				if (data.toString() == 'error') {
					...
					/* var html = "<div id=\""+json["fileName"]+"\" class=\"wrapper\"  style=\"cursor: pointer;\"><img style=\"width: 120px; height: 120px;margin:5px; background: #c2c2c2; cursor: pointer;\" src=\""+json["filePath"]+"\"/><div onclick=\"deleImg(this)\" class=\"divdelete\" style=\"display: block;\"></div></div>";
					$("#imgCet").append(html); */
					var html = "<div id=\""+json["fileName"]+"\" class=\"wrapper\"  style=\"cursor: pointer;\"><img style=\"width: 120px; height: 120px;margin:5px; background: #c2c2c2; cursor: pointer;\" src=\""+json["filePath"]+"\"/><div onclick=\"deleImg2(this)\" class=\"divdelete\" style=\"display: block;\"></div></div>";
					var height = $("#imgCet").height()+63;
					$("#imgCet").height(height);
					$("#imgCet").append(html);
				}
			}
		});
	...
	<div
		...
		<table id="table_chinese" class="table">
			...
			<tr>
				<td>认证证书：</td>
				<td>
				<div id="imgCet" style="height: auto;min-height: 100px">
					</div>
				<!-- <img id="imgCet" src="" width="120px" height="150px"> -->
					<div>
						<div id="queuecet"></div>
