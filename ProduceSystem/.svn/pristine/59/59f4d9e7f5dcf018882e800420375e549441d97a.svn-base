...
<script type="text/javascript">
	...
	$(document).ready(function(){
		var typs = '${apply.applicationType}';
		var arr = typs.split(',');
		$("#tb_apply_typs").children().each(function(){
		...
		if('${apply.isApplyOriginCode.trim()}' == '100011'){
			$("#radio_yes").attr("checked","checked");
		}else{
			$("#radio_no").attr("checked","checked");
		}
		if('${apply.applyOriginType.trim()}' == '1000161'){
			$("#type_weather").attr("checked","checked");
		}else{
			$("#type_pure_weather").attr("checked","checked");
		}
	});
	...
	function submit() {
        ...
        $("[name='chb_type']:checked").each(function(){
		 ...
		 $.ajax({
             type: "post",
             url: "<%=basePath%>/authc/editApply?applyBh=${applyBh}",
             dataType: "html",
	...
	function del_cert(thiz,id) {
		if(!confirm('是否删除该资质证明?')){
			return false;
		}
		$.ajax({
			type:"POST",
			url:"<%=basePath%>/authc/del_cert",
			data:{"id":id},
			success:function(data){
            	 if(data.success == "true"){
            		 alert("删除成功");
				...
				<tbody>
					<tr>
						...
						<td class="content_left"><input id="edit_company_name"
							name="edit_company_name" type="text" value=${apply.unityName }></td>
						<td class="text_right"><span class="required">*</span>通讯地址</td>
						<td class="content_left"><input id="edit_contact_address"
							name="edit_contact_address" type="text" value=${apply.address }></td>
					</tr>
					<tr>
					...
					<c:forEach items="${bsq}" var="item">
						<tr id="${item.certificateId}">
							<td><img height="50px" width="50px"
								src="<%=path %>${item.pictureUrl }" onclick="javascript:window.open(this.src);"></td>
							<td>${item.certificateName }</td>
							<td>${item.awardDepart }</td>
							<td>${item.getTime }</td>
							<td><a onclick="del_cert(this,'${item.certificateId}')">删除</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
						...
						<td class="content_left"><select id="select_type"
							name="select_type" style="width: 120px">
								<c:forEach items="${dicType}" var="dicItem">
									<option value="${dicItem.cuitMoonDictionaryCode}" >≡${ dicItem.cuitMoonDictionaryName}
									<c:if test="${apply.commodityType eq  dicItem.cuitMoonDictionaryCode.toString().trim()}">
										selected="selected"
									</c:if>
									</option>
